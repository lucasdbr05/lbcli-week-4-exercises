# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
transaction=""
address="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"

txid=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.txid')

inputs=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -c \
'[.vout | to_entries[] | {
  txid: "'$txid'",
  vout: .key,
  sequence: 0
}]')

outputs='{"'$address'": 0.2}'

locktime=$(echo "25 + (6*24*14)" | bc)

rawtx=$(bitcoin-cli -regtest createrawtransaction "$inputs" "$outputs" $locktime)

echo $rawtx